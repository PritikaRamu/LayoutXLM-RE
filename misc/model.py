...
#tokenizer.decode(dataset['validation']['input_ids'][0])

dataset['validation']['relations'][0]

dataset['validation']['entities'][0]
...
bi = {k: v[0] for k,v in batch.items()}
pil_img = to_img(torch.tensor(bi['hd_image'], dtype=torch.uint8))
draw = ImageDraw.Draw(pil_img)

scale_factor = 1024/1000
rels = bi['relations']
ents = bi['entities']
rels_by_ents = []

for rel_head, rel_tail in zip(rels['head'], rels['tail']):
    rels_by_ents.append((
        ents['start'][rel_head],
        ents['end'][rel_head],
        ents['start'][rel_tail],
...
for r00, r01, r10, r11 in rels_by_ents:
    rels_by_index[0].update({x for x in range(r00, r01)})
    rels_by_index[1].update({x for x in range(r10, r11)})

for i, (token, bbox) in enumerate(zip(bi['input_ids'], bi['bbox'])):
    if token != 1:
